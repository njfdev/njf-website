...
export default function BlogPreview({ title, description, publish_date, thumbnail, link, aboveFold }) {
    const router = useRouter()

    const formatted_date = new Date(publish_date).toLocaleDateString()

    return <li className="flex flex-col gap-2 bg-neutral-800 rounded-xl h-full p-5 cursor-pointer" onClick={() => {router.push(link)}}>
