        ...
        <div className={`flex select-none h-max ${containerClass}`} onClick={onClick}>
            <Link href={href}>
                <div className="flex cursor-pointer">
                    {icon ? <Icon path={icon} size={screenSize == ScreenSizes.small ? 1.5 : 1} className="text-stone-800 dark:text-stone-300 my-auto" /> : <></>}
                    <div className="flex flex-col">
                        <div className="my-auto" />
                        <span className={`text-neutral-800 dark:text-neutral-300 text-lg text-center align-middle font-extrabold ${textClass || (screenSize == ScreenSizes.small ? '!text-3xl' : '')}`}>
                            {children}
                        </span>
                        <div className="my-auto" />
                    </div>
